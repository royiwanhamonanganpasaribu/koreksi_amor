...
select in_log('koreksi_amor10.sql','Perubahan di amor_etap');
...
-- P R O V I S I 
  ...
  for acr in select kredit.*, amor_etap.*, nama_nas ,amor_bulanini
    ...
    where tgl_mediator_krd = sekarang() and amor_etap.id_je = 'M' and kredit.typebunga_krd = 'A'  order by akta_krd
  loop
    if nojur = 0 then
      nojur := nextval('transjr_id_jr_seq');
      bt := no_ledger('77');
